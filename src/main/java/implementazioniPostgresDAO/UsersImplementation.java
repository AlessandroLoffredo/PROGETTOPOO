    ...
    public void getOrganizers(ArrayList<String> organizers) throws SQLException{
        try(Connection conn = ConnessioneDatabase.getInstance().connection){
            String sql = "SELECT P.username FROM PlUser P WHERE P.username NOT IN (" +
                    ...
                    "WHERE O.idHack = H.idHack AND H.endDate >= CURRENT_DATE) AND " +
                    "P.Username NOT IN (" +
                    "SELECT J.username FROM Judge J, Hackathon H " +
                    "WHERE J.idHack = H.idHack AND H.endDate >= CURRENT_DATE) AND " +
                    "P.username NOT IN (" +
                    "SELECT Pa.username FROM Participant Pa, Hackathon H, Team T " +
                    "WHERE Pa.idTeam = T.idTeam AND T.idHack = H.idHack AND H.endDate >= CURRENT_DATE)";
